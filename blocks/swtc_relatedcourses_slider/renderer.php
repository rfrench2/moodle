...
/**
 *
 * SWTC LMS for Moodle 3.7+. Renderer definitions for SWTC LMS related course slider block.
 *
...
class block_swtc_relatedcourses_slider_renderer extends plugin_renderer_base {
    ...
    public function block_swtc_relatedcourses_slider_add_instance_css($instancecssid, $displayoptionscss) {
        ...
        // print_object($instancecss);     // 09/25/19
    ...
    public function block_swtc_relatedcourses_slider_relatedcourseslider_as_html($mappedcourses, $coursesorder, $instancecssid, $displayoptionshtml) {
        global $CFG, $COURSE, $USER;       // 08/14/19
        $relatedcourseslider = '';
        ...
        $coursepictures = '';
        $coursesummary = '';
        $summary_trunc_length = 200;      // 09/17/19 - Lenovo
        $coursename = '';
        $courseurl = '';
        $coursecount = 0;
        ...
        foreach ($coursesorder as $id) {
            if (array_key_exists ( $id, $mappedcourses )) {
                ...
                $coursehtml = '';
                $coursenavhtml = '';
                $clickparams = array();     // 08/14/19

                $clickparams['action'] = 'click';               // 08/27/19
                $clickparams['type'] = 'related';               // 08/23/19
                $clickparams['parentcourseid'] = $COURSE->id;       // 08/14/19
                $clickparams['clickedcourseid'] = $id;                      // 08/14/19
                $clickparams['clickeduserid'] = $USER->id;              // 08/14/19
                ...
                if ($displayoptionshtml->coursesummaryflag && $course->has_summary ()) {
                    ...
                    // Lenovo ********************************************************************************.
                    // 09/17/19 - If for some reason the length requested is equal to or is greater than the current length
                ...
                if ($displayoptionshtml->coursenameflag) {
                    ...
                    $coursename = $course->shortname . ' ' . $coursename;           // 08/01/19 Lenovo
        ...
        // print_object($relatedcourseslider);      // 08/14/19 Lenovo
    ...
    public function block_swtc_relatedcourses_slider_pictures_as_html($course) {
        ...
        if (empty ( $coursepictures )) {
            ...
            if ($file) {
                ...
                // Lenovo ********************************************************************************.
                // 11/11/19 - Changed "defaultimage" to "alldefaultimage" (for all courses except PA courses) and added
