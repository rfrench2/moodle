...
/**
 * Renderer for outputting the ebglmscustom course format.
 *
 ...
 * Lenovo history:
 *
 *  04/11/17 - Copied format_summary_text from /course/format/renderer for modification; added courseverison and machinetypes field to form.
 *  04/17/17 - Removed (or commented-out) courseversion (not implementing now).
 * 04/06/19 - Removed numsections.
...
/**
 * Basic renderer for ebglmscustom format.
 *
 * @copyright 2017 Lenovo EBG LMS
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_ebglmscustom_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
       ...
       // Lenovo - wish this would work...
	   //$this->courserenderer = new format_ebglmscustom_course_renderer($page, $target);

		// Since format_ebglmscustom_renderer::section_edit_controls() only displays the 'Set current section' control when editing mode is on
    ...
    protected function start_section_list() {
        return html_writer::start_tag('ul', array('class' => 'ebglmscustom'));
    }
    ...
    protected function format_summary_text($section) {
        ...
        // @01 - 04/28/20 - Moved some DCG custom course format strings to /local/ebglms/lang/en/local_ebglms.php
        //                  to remove duplication of common strings.
        $header_string_1 = get_string('overview_coursetitle_line1_formatting', 'local_ebglms');
        $header_string_2 = get_string('overview_coursecode_line2_formatting', 'local_ebglms');
        // 04/17/17 - Removed (or commented-out) courseversion (not implementing now).
        // $header_string_3 = get_string('overview_currentversion_line3_formatting', 'local_ebglms');
        $header_string_4 = get_string('overview_machinetypes_line4_formatting', 'local_ebglms');
        // 08/12/19
        $header_string_5 = get_string('overview_duration_line5_formatting', 'local_ebglms');
        $header_string_6 = get_string('overview_heading_line6_formatting', 'local_ebglms');
        $header_string_7 = $COURSE->summary;
        $header_string_emptyline = '<br />';

        // For searching for current version of course.
        // 04/17/17 - Removed (or commented-out) courseversion (not implementing now).
        // $overview_version = get_string('overview_version', 'format_ebglmscustom');
        ...
        // Lenovo *******************************************************************************
        if (isset($debug)) {
            $messages[] = "Entering format_summary_text in ebglmscustomformat/renderer.php ===88.enter===.";
            debug_logmessage($messages, 'both');
            unset($messages);
        ...
        // Lenovo *******************************************************************************
        if (isset($debug)) {
            $messages[] = "Leaving format_summary_text in ebglmscustomformat/renderer.php ===88.leaving===.";
            debug_logmessage($messages, 'both');
            unset($messages);
    ...
    public function render_htmlpage(htmlpage $app) {
        ...
        return $this->render_from_template('format_ebglmscustom/htmlpage', $context);
    }
}
