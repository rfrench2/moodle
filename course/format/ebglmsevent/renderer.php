...
/**
 * Renderer for outputting the ebglmsevent course format.
 *
...
use \format_ebglmsevent\output\htmlpage;

/**
 * Basic renderer for ebglmsevent format.
 *
 * @copyright 2020 Lenovo EBG LMS
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_ebglmsevent_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
	   ...
	   //$this->courserenderer = new format_ebglmsevent_course_renderer($page, $target);
		
		// Since format_ebglmsevent_renderer::section_edit_controls() only displays the 'Set current section' control when editing mode is on
    ...
    protected function start_section_list() {
        return html_writer::start_tag('ul', array('class' => 'ebglmsevent'));
    }
    ...
    protected function format_summary_text($section) {
        ...
        $header_string_1 = get_string('overview_eventtitle_line1_formatting', 'format_ebglmsevent');
        $header_string_2 = get_string('overview_eventcode_line2_formatting', 'format_ebglmsevent');        
        // 04/17/17 - Removed (or commented-out) courseversion (not implementing now).
        // $header_string_3 = get_string('overview_currentversion_line3_formatting', 'format_ebglmsevent');        
        $header_string_4 = get_string('overview_machinetypes_line4_formatting', 'format_ebglmsevent');
        // 08/12/19
        $header_string_5 = get_string('overview_duration_line5_formatting', 'format_ebglmsevent');
        $header_string_6 = get_string('overview_heading_line6_formatting', 'format_ebglmsevent');
        $header_string_7 = $COURSE->summary;
        $header_string_emptyline = '<br />';
        
        // For searching for current version of course.
        // 04/17/17 - Removed (or commented-out) courseversion (not implementing now).
        // $overview_version = get_string('overview_version', 'format_ebglmsevent');
        ...
        // Lenovo *******************************************************************************
        if (isset($debug)) {
            $messages[] = "Entering format_summary_text in ebglmseventformat/renderer.php ===88.enter===.";
            debug_logmessage($messages, 'both');
            unset($messages);
        ...
        if ($section_number ===0) {
            ...
            $header_string_1 = str_replace("string-eventfullname", $COURSE->fullname, $header_string_1);
        ...
        // Lenovo *******************************************************************************
        if (isset($debug)) {
            $messages[] = "Leaving format_summary_text in ebglmseventformat/renderer.php ===88.leaving===.";
            debug_logmessage($messages, 'both');
            unset($messages);
    ...
    public function render_htmlpage(htmlpage $app) {
        ...
        return $this->render_from_template('format_ebglmsevent/htmlpage', $context);
    }
}
