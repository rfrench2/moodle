 ...
 * @since Moodle 2.3
 */
class format_swtcpractical extends format_base {
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_swtcpractical');
        } else {
            // Use format_base::get_default_section_name implementation which
            // will display the section name in "Topic n" format.
            return get_string('sectionname', 'format_swtcpractical');
        }
    }
    ...
    public function get_default_blocks() {
        return array(
            ...
            BLOCK_POS_RIGHT => array('course_slider')
        );
    }
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            ...
            $courseformatoptions = array(
                ...
                /* SWTC LMS practical format */
                'coursetype' => array(
                    'default' => get_config('format_swtcpractical', 'coursetype'),
                    'type' => PARAM_INT,
                ),
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            $courseformatoptionsedit = array(
                ...
                'coursetype' => array(
                    'label' => new lang_string('coursetype', 'format_swtcpractical'),
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(
                            COURSETYPE_SERVICEPROVIDER => get_string('serviceprovider_portfolio', 'format_swtcpractical'),
                            COURSETYPE_GTP => get_string('gtp_portfolio', 'format_swtcpractical'),
                            COURSETYPE_IBM => get_string('ibm_portfolio', 'format_swtcpractical'),
                            COURSETYPE_LENOVO => get_string('lenovo_portfolio', 'format_swtcpractical'),
                            COURSETYPE_LENOVOINTERNAL => get_string('lenovointernal_portfolio', 'format_swtcpractical'),
                            COURSETYPE_LENOVOSHAREDRESOURCES =>
                                get_string('lenovosharedresources_portfolio', 'format_swtcpractical'),
                            COURSETYPE_MAINTECH => get_string('maintech_portfolio', 'format_swtcpractical'),
                            COURSETYPE_ASP => get_string('asp_portfolio', 'format_swtcpractical'),
                            COURSETYPE_PREMIERSUPPORT => get_string('premiersupport_portfolio', 'format_swtcpractical'),
                            COURSETYPE_SERVICEDELIVERY => get_string('servicedelivery_portfolio', 'format_swtcpractical'),
                            COURSETYPE_PRACTICALACTIVITIES => get_string('practicalactivities_portfolio', 'format_swtcpractical')
                        )
                    ),
                    'help' => 'coursetype',
                    'help_component' => 'format_swtcpractical',
                ),
                /* SWTC LMS iscurriculum field */
                'iscurriculum' => array(
                    'label' => new lang_string('iscurriculum', 'format_swtcpractical'),
                ...
                /* SWTC LMS ispartofcurriculum field */
                'ispartofcurriculum' => array(
                    'label' => new lang_string('ispartofcurriculum', 'format_swtcpractical'),
                    'element_type' => 'advcheckbox',
                ...
                /* SWTC LMS curriculums field */
                'curriculums' => array(
                    'label' => new lang_string('curriculums', 'format_swtcpractical'),
                    'element_type' => 'select',
                    'element_attributes' => array(
                        $curriculumarray,
                        'multiple' => 'multiple',
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        $mform->insertElementBefore($mform->createElement('text', 'machinetypes',
            get_string('machinetypes', 'format_swtcpractical'), ''), 'category');
        $mform->addHelpButton('machinetypes', 'machinetypes', 'format_swtcpractical');
        ...
        $mform->insertElementBefore($mform->createElement('text', 'duration',
            get_string('duration', 'format_swtcpractical'), ''), 'category');
        $mform->addHelpButton('duration', 'duration', 'format_swtcpractical');
     ...
     *
     * In case if course format was changed to 'swtcpractical', we try to copy options
    ...
    public function update_course_format_options($data, $oldcourse = null) {
        ...
        if ($oldcourse !== null) {
            ...
            if (isset($debug)) {
                ...
                $messages[] = "Finished printing options. About to print data (only includes definition of options,
                    not actual data).";
            ...
            // In update_course_format_options, if course format option does not exist in $data, do NOT zero out the value.
            // SWTC ********************************************************************************.
            ...
            // SWTC ********************************************************************************.
            // Added related courses listbox to /course/edit_form.php; added related courses to course format options.
            // Notes:
            ...
            // it must be handled differently because it is shown in the "General" section, not the "Course format"
            // section. Load form element value with 'machinetypes' from database (if it exists).
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_swtcpractical');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_swtcpractical', $title);
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        $renderer = $PAGE->get_renderer('format_swtcpractical');
 ...
 *
 */
function format_swtcpractical_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'swtcpractical'), MUST_EXIST);
