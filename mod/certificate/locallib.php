 ...
 * Lenovo history:
 *
 *	04/11/19 - Added this header.
 * 05/28/19 - In certificate_email_student, between either the move to 3.6.3 or Motorola hosting, it seems to be selecting the "teacher" 
 *                      differently (even though the "main" site admin is set, it is using "Lauren Chapman" instead;
 *                      commented out "teacher" code to always use get_admin.
 *
 */
 ...
 * Lenovo history:
 *
 * 05/28/19 - Added this header; between either the move to 3.6.3 or Motorola hosting, it seems to be selecting the "teacher" 
 *                      differently (even though the "main" site admin is set, it is using "Lauren Chapman" instead;
 *                      commented out "teacher" code to always use get_admin.
 *
 */
function certificate_email_student($course, $certificate, $certrecord, $context, $filecontents, $filename) {
    ...
    // Get teachers
    // Lenovo ********************************************************************************.
    // 05/28/19 - Commented out "teacher" code to always use get_admin.
 ...
 * Lenovo history:
 *
 *	04/11/19 - Added this header; changed pix_url to image_url.
 *
 */
function certificate_print_user_files($certificate, $userid, $contextid) {
    ...
    foreach ($files as $file) {
        ...
        // $output = '<img src="'.$OUTPUT->pix_url(file_mimetype_icon($file->get_mimetype())).'" height="16" width="16" alt="'.$file->get_mimetype().'" />&nbsp;'.      // Lenovo
        $output = '<img src="'.$OUTPUT->image_url(file_mimetype_icon($file->get_mimetype())).'" height="16" width="16" alt="'.$file->get_mimetype().'" />&nbsp;'.       // Lenovo
