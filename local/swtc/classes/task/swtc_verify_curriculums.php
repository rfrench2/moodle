...
/**
 * A scheduled task for Lenovo DCG Services Education. Used to add cron job to Moodle for verifying curriculums.
 *
 * Version details
 *
 * @package    swtc_verify_curriculums
 ...
 * 11/30/18 - Adding fully qualified path to Moosh.
 * 12/04/18 - Changed parameters to match the Lenovo Moosh command.
 *
 **/
 // Lenovo ********************************************************************************
namespace local_swtc\task;

class swtc_verify_curriculums extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('crontaskverifycurriculums', 'local_swtc');
    }
    ...
    public function execute() {
        ...
        $log_filename = $log_folder . 'swtc_verify_curriculums.debug' .".". $file_ext;
        ...
        $moosh = $fqpath . 'moosh -n -v lenovo-curriculums --deb %';

        // Lenovo ********************************************************************************
        // Strings required for header and footer. Always output standard header information.
        // Lenovo ********************************************************************************
        $header = "Lenovo ********************************************************************************.\n";
        $header .=  "Running scheduled task LenovoCurriculums. Entering swtc_verify_curriculums.execute.enter===.\n";

        $footer = "Running scheduled task LenovoCurriculums. Leaving swtc_verify_curriculums.execute.exit===.\n";
        $footer .= "Lenovo ********************************************************************************.\n";
