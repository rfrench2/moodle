 ...
 */
class swtc_course_completion_form extends course_completion_form {
    ...
    public function definition() {
        ...
        $aggregationmethods = $completion->get_aggregation_methods();
        ...
        foreach ($aggregationmethods as $methodcode => $methodname) {
        ...
        $mform->addElement('select', 'overall_aggregation', get_string('overallaggregation', 'core_completion'),
            $overallaggregationmenu);
        ...
        if (!empty($activities)) {
            ...
            if (!$completion->is_course_locked()) {
                // SWTC ********************************************************************************.
                // Added code from previous version (for togglebetweenstates).
                // $this->add_checkbox_controller(1, null, null, 0);
                // SWTC ********************************************************************************.
                $togglerid = html_writer::random_id();                              // SWTC.
            ...
            if (count($activities) > 1) {
                ...
                foreach ($aggregationmethods as $methodcode => $methodname) {
                    if ($methodcode === COMPLETION_AGGREGATION_ALL) {
                        $activityaggregationmenu[COMPLETION_AGGREGATION_ALL] = get_string('activityaggregation_all',
                            'core_completion');
                    } else if ($methodcode === COMPLETION_AGGREGATION_ANY) {
                        $activityaggregationmenu[COMPLETION_AGGREGATION_ANY] = get_string('activityaggregation_any',
                            'core_completion');
                    } else {
                        $activityaggregationmenu[$methodcode] = $methodname;
                    }
                }
                $mform->addElement('select', 'activity_aggregation', get_string('activityaggregation', 'core_completion'),
                    $activityaggregationmenu);
        ...
        if ($hasselectablecourses) {
            ...
            $mform->addElement('select', 'course_aggregation', get_string('courseaggregation', 'core_completion'),
                $courseaggregationmenu);
        ...
        $coursegrade = $DB->get_field('grade_items', 'gradepass', array('courseid' => $course->id, 'itemtype' => 'course'));
        if (!$coursegrade) {
            $coursegrade = '0.00000';
        }
        $criteria = new completion_criteria_grade($params);
        $criteria->config_form_display($mform, $coursegrade);
        ...
        if (!empty($roles)) {
            foreach ($roles as $role) {
                $paramsa = array('role' => $role->id);
                $criteria = new completion_criteria_role(array_merge($params, $paramsa));
            ...
            $mform->addElement('select', 'role_aggregation', get_string('roleaggregation', 'core_completion'),
                $roleaggregationmenu);
