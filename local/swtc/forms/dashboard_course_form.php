...
class dashboard_course_form extends moodleform {
    function definition() {
        global $USER, $CFG, $DB;
        ...
        // Lenovo ********************************************************************************
        $header_string = "Course dashboard";
        $mform->addElement('header', 'sitecourses', $header_string);

        $mform->addElement('static', 'now', "Time : " .$this->_customdata['now']."");

        $header_string = "Enrollments";
        $mform->addElement('header', 'enrollments', $header_string);

        $mform->addElement('static', 'alltime', 'All time :', $this->_customdata['alltime']);
        $mform->addElement('static', 'alltime1', '', "This is the total number of unique course enrollments that have occurred since the site began.");

        $mform->addElement('static', 'lastyear', 'Last year :', $this->_customdata['lastyear']);
        $mform->addElement('static', 'lastyear1', '', "This is the total number of unique course enrollments that occurred last year.");

        $mform->addElement('static', 'thisyear', 'So far this year :', $this->_customdata['thisyear']);
        $mform->addElement('static', 'thisyear1', '', "This is the total number of unique course enrollments that occurred this year.");
