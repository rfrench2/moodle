 ...
 * History:
 *
 *	10/23/18 - Initial writing; all curriculum utility functions.
 ...
 *                          users from appearing; fixed table output if no users are listed.
 * 12/14/18 - Added curriculum_courses_list (lists all the courses in either all the curriculums or a single curriculum).
 * 12/15/18 - Added curriculum_courses_find_course and curriculum_is_user_enrolled.
 * 12/18/18 - Added curriculums_getall_enrollments_for_user (lists all the curriculum courses the user is enrolled in, if any).
 ...
 * 05/10/19 - In curriculum_report_completion_index, in the url links to the other course completion reports, use the user's group in that
 *                      course, not the curriculum course.
 * 05/18/19 - Updated curriculum_report_completion_index if no group is returned (user is either Lenovo-admin or Lenovo-siteadmin).
 *	10/16/19 - Changed to new Lenovo EBGLMS classes and methods to load swtc_user and debug.
 * 10/22/19 - Added lenovo_set_where_conditions_by_accesstype.
 * 10/28/19 - Changed get_details to lenovo_get_details_course (defined in /local/swtc/traits\lenovo_completion_criteria).
 * 11/11/19 - In curriculum_report_completion_index, added $swtc_user to parameters to function; when callling
 *                          lenovo_set_where_conditions_by_accesstype, added $swtc_user as parameter to function.
 * 12/12/19 - In curriculum_courses_list, changed first returned parameter from "courseid" to "id" (to mirror parameters returned
 *                          from $DB->get_records).
 * 12/30/19 - When calling lenovo_set_where_conditions_by_accesstype, if $where or $whereparams was set when these functions
 *                       were called, pass them along and preserve them.
...
// Lenovo ********************************************************************************.
// Lenovo customized code for Moodle core completion.
// Lenovo ********************************************************************************.
 ...
 * 11/28/18 - Changed specialaccess to viewcurriculums; split accessreports into accessmgrreports and accessstudreports;
 *                      removed all three and changed to customized capabilities.
 * 11/29/18 - Removed test users from appearing in reports (users with firstname like %test%); fixed table output if no users are listed.
    ...
    if (!$csv) {
        ...
        if (COMPLETION_REPORT_COL_TITLES) {
            ...
            foreach ($criteria as $criterion) {
                ...
                // Lenovo ********************************************************************************
                // list($shortname, $fullname) = $criterion->get_title_detailed();		// Lenovo
                ...
                // $courseurl = new moodle_url("/course/view.php", array('idnumber' => $shortname));		// Lenovo
                ...
                // $output .= '<div class="rotated-text-container"><span class="rotated-text">'.$courseurl.'</span></div>'; // Lenovo - original - SAVE!
				$output .= '<div class="rotated-text-container"><span class="rotated-text"><a href="'.$courseurl->out().'" target="_blank" title=" '.$tooltiptext.' ">'.$shortname.'</a></span></div>';		// Lenovo
        ...
        foreach ($criteria as $criterion) {
            ...
            if ($criterion->criteriatype == COMPLETION_CRITERIA_TYPE_ACTIVITY) {
                ...
                // $row[] = strip_tags($criterion->get_title_detailed());		// Lenovo
                // Lenovo ********************************************************************************.
                // 12/20/18 - Added hyperlinks and tooltips to the column headers.
                // 03/20/19 - Changing tooltips hyperlink from viewing course to viewing individual course completion report.
                //              All of the following must be kept in sync:
