...
/**
 * Initializes all debugging options related to $SESSION->SWTC->DEBUG->user (lower case).
 *
...
/**
 * Used to start debugging. Returns $SESSION->SWTC->DEBUG.
 *
 * @param N/A
 *
 * @return $array   The DEBUG array.
...
function debug_start() {
    ...
    //      To use: $SESSION->SWTC or $SESSION->{'EBGLMS'}->{'DEBUG'}
    // SWTC ********************************************************************************
    if (!isset($SESSION->SWTC->DEBUG)) {
        // SWTC ********************************************************************************
        // Setup $SESSION->SWTC->DEBUG and set a temporary reference to it.
        //      To use: $debug = $SESSION->SWTC->DEBUG
        // SWTC ********************************************************************************
        // print_r("debug_start: SESSION->SWTC->DEBUG ->NOT<- set. Called from ".debug_backtrace()[1]['function'].".<br />");
        // If running on production, check the authorized user list.

        $debug = debug_setup();

        $SESSION->SWTC->DEBUG = $debug;

        // SWTC ********************************************************************************
        // Setup the third-level $PHPLOG global variable.
        //      To use: $SESSION->SWTC->DEBUG->PHPLOG
        // SWTC ********************************************************************************
        // $php_log = debug_setup_phplog();                                                // 04/20/18: See above.
        // $SESSION->SWTC->DEBUG->PHPLOG = $php_log;      // 04/20/18: See above.
        $SESSION->SWTC->DEBUG->PHPLOG = null;

        // SWTC ********************************************************************************
        // Setup the fourth-level $user global variable.
        //      IMPORTANT! This in only for debugging to track if and when values in $SESSION->SWTC->USER get initially set
        //              or change.
        //      To use: $debug_user = $SESSION->SWTC->DEBUG->user;
        // SWTC ********************************************************************************
        // $debug->user = debug_setup_user($SESSION->SWTC->USER);  // 04/26/18 - RF - ***here***
        ...
        // $SESSION->SWTC->DEBUG->PHPLOG->backtrace = format_backtrace(debug_backtrace(), true);      // 07/19/18
        // print_r("SESSION->SWTC->DEBUG follows");
        // var_dump($SESSION->SWTC->DEBUG);
        // die;
        // print_r("debug_start: SESSION->SWTC->DEBUG =IS= set. Called from ".debug_backtrace()[1]['function'].".<br />");

        $SESSION->SWTC->DEBUG->information = 'In debug_start() - DEBUG is already set.';

        // Go ahead and update the PHP log information for this call.
        if (isset($SESSION->SWTC->DEBUG->PHPLOG)) {
            $today = new DateTime("now", core_date::get_user_timezone_object());
            $SESSION->SWTC->DEBUG->PHPLOG->backtrace = format_backtrace(debug_backtrace(), true);     // true generates text; false generates HTML.
            $SESSION->SWTC->DEBUG->PHPLOG->timeofevent = $today->format('H:i:s.u');
        }

        $SESSION->SWTC->DEBUG->information = 'In debug_start() - Returning...';
...
function debug_load_includes($option = 'swtc') {
    ...
    //      To use: $includes = $SESSION->SWTC->DEBUG->includes;
    // SWTC ********************************************************************************
    if (!isset($SESSION->SWTC->DEBUG)) {
        ...
        $SESSION->SWTC->DEBUG->includes = $includes;
        return;
...
function debug_authorized_user($username) {
   ...
   // Local variables begin...
    ...
    $debug_usernames = array('rfrench@lenovo.com',
                            ...
                            'test-sdem1-admin1',
                            'test-sdem-geoadmin1',
                            'test-lenovo-stud',                         // SWTC
                            'test-lenovo-admin',                         // SWTC
                            'test-lenovo-siteadmin',                         // SWTC
                            'test-ibm-stud',                        // IBM
                            // 'admin',
                            'override'
                            );
...
function debug_enable_phplog($debug, $text = null) {
    ...
    if (!isset($SESSION->SWTC->DEBUG->PHPLOG)) {
        ...
        $phplog = $SESSION->SWTC->DEBUG->PHPLOG;
    }
...
/**
 * Get username (returns $username). REQUIRES $SESSION->SWTC->DEBUG to be set.
 */
...
/**
 * Use to query if debugging has been started (returns value of isset($SESSION->SWTC->DEBUG)); true = debug is running;
 *                  false = not running.
...
function debug_running() {
    ...
    //      To use: $debug = $SESSION->SWTC->DEBUG;
    // SWTC ********************************************************************************
    return (isset($SESSION->SWTC->DEBUG));
}

/**
 * Get the log file name (gets either $debug_fqlog or $debug_fqdetailed). REQUIRES $SESSION->SWTC->DEBUG to be set.
 */
...
/**
 * Set the fully qualified log file names (sets either $debug_fqlog or $debug_fqdetailed). REQUIRES $SESSION->SWTC->DEBUG to be set.
 *
 * @param N/A
 *
 * @returns N/A
