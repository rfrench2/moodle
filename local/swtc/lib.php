 ...
 * 05/11/16 - v10 - Added new category, capabilities, etc. for Maintech.
 ...
 * 02/27/17 - v14 (fix) - Starting with Moodle 3.1, searching for $sharedresources_coursename alone does not work. Now have to search for,
 *									and remove, $leninternalsharedresources also.
 *	03/29/17 - To show students grades (Userid > Grades), removed call to load_blocks (was causing a blank screen for everyone).
 * 11/09/17 - Changed "course_overview" for Moodle < 3.3 to "myoverview" for Moodle >= 3.3.
 * 04/11/18 - Added new $SESSION->EBGLMS global variables and all its required changes.
 ...
 * 05/21/18 - Due to changes in using roles versus removing menu items, removing local_swtc_extend_settings_navigation and
 *                      local_swtc_extend_navigation_user_settings.
 * 05/24/18 - Added PremierSupport-administrator role for checking access; if PremierSupport-admin or PremierSupport-manager,
 *                      we must remove the "Users->Permissions" node here because both roles need to have the "moodle/role:assign"
 *                      permission so that they can view/edit cohort enrollment instances.
 * 05/26/18 - Added back local_swtc_extend_settings_navigation to check for Frontpage node (so it can be removed).
 ...
 * 09/05/18 - When checking for Site administration menu, adding check for Lenovo-administrators.
 * 11/15/18 - Added check for "specialaccess" flag.
 * 11/20/18 - Added check for Category settings node. If found, remove for all users.
 ...
 * 12/28/18 - Added 'group' parameter to URL that is built for each tab for PremierSupport and ServiceDelivery managers and administrators;
 *						removing Site administration > Reports > Statistics using role definitions.
 ...
 * 02/15/19 - Adding moodle/site:config to Lenovo-siteadmin and Lenovo-admin so that they get access to local_course_templates
 *                           and Lenovo Services Education site menu options; remove Site administration menu options that not all administrators
 *                           need.
 ...
 * 03/06/19 - Added PS/AD GEO administrator user access types.
 * 03/31/19 - Removed all customized code referencing "PremSuppTempCourse".
 * 04/10/19 - Removed Site administration > Reports for PS / SD access types.
 * 05/09/19 - Added additional code for Lenovo-admins and Lenovo-siteadmins.
 * 05/13/19 - Updated local_swtc_extend_navigation_course to correct the course participation hyperlink (changed "course" to "id").
 * 06/25/19 - For Lenovo admin user types, enable more Site administration menu items (/local/swtc/lib.php).
 ...
 * 10/22/19 - For Lenovo administrators, adding back in several Course Administration and Site Administration menu items; in Moodle 3.7+,
 *                      removing "Site Administration > Messaging" menu".
 * 11/04/19 - In local_swtc_extend_navigation, removed "courses" node.
 * 11/06/19 - Lenovo EBGLMS for Moodle 3.7+; to solve missing definitions elsewhere, changing require_once to require for swtc.php;
 *                      changes to set Lenovo EBGLMS swtc_user and debug variables.
 * 11/14/19 - In local_swtc_extend_navigation, added fix to remove "lenovoedu > Participants" navigation node if not a
 *                      Lenovo-admin or Lenovo-siteadmin.
 * 12/03/19 - Removing Site administration menu for all PremierSupport and ServiceDelivery access types (since cohorts are defined
 *                      at the System level, we must keep the PS/SD System context. Therefore, we must manually remove the Site
 *                      administration menu).
...
require_once($CFG->dirroot.'/local/swtc/lib/curriculumslib.php');         // Some needed Curriculum functions.
 ...
 *                      removing "Site Administration > Messaging" menu".
 * 11/04/19 - In local_swtc_extend_navigation, removed "courses" node.
 * 11/06/19 - Lenovo EBGLMS for Moodle 3.7+; changes to set Lenovo EBGLMS swtc_user and debug variables.
 * 11/14/19 - In local_swtc_extend_navigation, added fix to remove "lenovoedu > Participants" and navigation node if not
 *                      a Lenovo-admin or Lenovo-siteadmin.
 * 03/02/20 - In local_swtc_extend_navigation, added call to core_course_category::make_categories_list with the
 *                      user's main capability for easier checking of access (before moving to core_course_category::can_view_category).
...
function local_swtc_extend_navigation(global_navigation $nav) {
	global $USER, $PAGE, $CFG, $DB, $OUTPUT, $SESSION;
    ...
    $site = 'site';                                                                   // The key for 'Site' (i.e. Navigation > Home > lenovoedu).
    $participants = 'participants';                                         // The key for 'Participants' (i.e. Navigation > Home > lenovoedu > Participants).
    ...
    // SWTC ********************************************************************************
    // Main loop. See if children is in $SESSION->EBGLMS->USER->categoryids. If it is found, the user has access, so
    //                  leave the course in the list. If the user doesn't, remove it from the list.
    // SWTC ********************************************************************************
    foreach ($children as $key => $catid) {
        //  if (isset($debug)) {
        //      $child = $courses->get($children[$key]);
        //      $messages[] = "About to print child category $catid (index position $key). ===1.1===.";
        //      $messages[] = print_r($child, true);
        //      $messages[] = "Finished printing print child category $catid (index position $key). ===1.1===.";
        //      debug_logmessage($messages, 'detailed');
        //      unset($messages);
        //  }
        // $childcat->id = $child->id;
        // print("children[key] is :$children[$key]");
        // if (swtc_user_access_category($catid)) {       // 03/02/20
        if ((in_array($catid, array_keys($categories))) || (stripos($swtc_user->user_access_type, $access_selfsupport_stud) !== false)) {
            if (isset($debug)) {
                $messages[] = "Child category $catid found in SESSION->EBGLMS->USER->categoryids. Keeping category in list. ===1.1===.";
                debug_logmessage($messages, 'detailed');
                unset($messages);
            }
        } else {
            if (isset($debug)) {
                $messages[] = "Child category catid NOT found in SESSION->EBGLMS->USER->categoryids. Removing category from list. ===1.1===.";
                debug_logmessage($messages, 'detailed');
                unset($messages);
            }
    ...
    // SWTC ********************************************************************************.
    // 11/14/19 - In local_swtc_extend_navigation, added fix to remove "lenovoedu > Participants" navigation node if not a Lenovo-admin
    //                      or Lenovo-siteadmin.
    // SWTC ********************************************************************************.
    // SWTC ********************************************************************************
    if ((!preg_match($access_lenovoadmin, $user_access_type)) && (!preg_match($access_lenovositeadmin, $user_access_type))) {
   ...
   // $node->add($mycurriculums, new moodle_url('/my/mycurriculums'), self::TYPE_CUSTOM, null, 'mycurriculums');

	// SWTC ********************************************************************************
	// Looking for 'Lenovo Shared Resourses (Master)' course in 'My courses'. If found, remove it.
	//		Note: Course short name for course is "LenInternalSharedResources".
	// SWTC ********************************************************************************
	// Search for "LenInternalSharedResources" course id in "My courses".
	$sharedresources_node = $nav->find($swtc_resources->sharedres_courseid, null);

	if ( !empty($sharedresources_node)) {
		if (isset($debug)) {
			$messages[] = "I found $swtc_resources->sharedres_shortname in My courses ===1.2===.";
            //  $messages[] = "About to print sharedresources_node ==1===.";
            //  $messages[] = print_r($sharedresources_node, true);
            //  $messages[] = "Finished printing sharedresources_node ==1===.";
            debug_logmessage($messages, 'logfile');
            unset($messages);
		}
		if ( !$sharedresources_node->remove()) {
			if (isset($debug)) {
				debug_logmessage("Error removing $swtc_resources->sharedres_shortname from My courses ===1.2===.", 'logfile');
			}
		} else {
			if (isset($debug)) {
				debug_logmessage("Successfully removed $swtc_resources->sharedres_shortname from My courses ===1.2===.", 'logfile');
			}
		}
	}else {
		if (isset($debug)) {
			debug_logmessage("I DIDN'T find $swtc_resources->sharedres_shortname in My courses ===1.2===.", 'logfile');
		}
	}

    // SWTC ********************************************************************************
	// Looking for shared simulator course in 'My courses'. If found, remove it.
	//		Note: Course short name for course is "lensharedsimulators_shortname".
	// SWTC ********************************************************************************
	// Search for "lensharedsimulators_shortname" course id in "My courses".
	$sharedresources_node = $nav->find($swtc_resources->lensharedsimulators_courseid, null);

	if ( !empty($sharedresources_node)) {
		if (isset($debug)) {
			$messages[] = "I found $swtc_resources->lensharedsimulators_shortname in My courses ===1.2===.";
            //  $messages[] = "About to print sharedresources_node ==1===.";
            //  $messages[] = print_r($sharedresources_node, true);
            //  $messages[] = "Finished printing sharedresources_node ==1===.";
            debug_logmessage($messages, 'logfile');
		...
		if ( !$sharedresources_node->remove()) {
			if (isset($debug)) {
				debug_logmessage("Error removing $swtc_resources->lensharedsimulators_shortname from My courses ===1.2===.", 'logfile');
			}
		} else {
			if (isset($debug)) {
				debug_logmessage("Successfully removed $swtc_resources->lensharedsimulators_shortname from My courses ===1.2===.", 'logfile');
			}
		}
	}else {
		if (isset($debug)) {
			debug_logmessage("I DIDN'T find $swtc_resources->lensharedsimulators_shortname in My courses ===1.2===.", 'logfile');
		}
	}
 ...
 * History:
 *
 * 02/13/19 - Added this header; fixed access for Lenovo-admin and Lenovo-siteadmin user types.
 * 02/15/19 - Adding moodle/site:config to Lenovo-siteadmin and Lenovo-admin so that they get access to local_course_templates
 *                           and Lenovo Services Education site menu options; remove Site administration menu options that not all administrators
 *                           need.
 * 03/03/19 - Added PS/AD site administrator user access types.
...
function local_swtc_extend_settings_navigation(settings_navigation $settingsnav, context $context) {
	global $USER, $PAGE, $CFG, $DB, $COURSE, $SESSION;
    ...
    $frontpage = 'frontpage';
    $admin_frontpage_remove = array($frontpage);
    $isfrontpage = false;
    ...
    $registrationmoodleorg = 'registrationmoodleorg';
	...
	// $admin_top_remove = array($adminnotifications, $registrationmoodleorg, $advancedfeatures, $grades, $analytics, $competencies, $location, $language, $modules, $security, $appearance, $frontpage, $server, $reports, $mobileapp, $development, $assignmentupgrade, $toolmultilangupgrade, $moodleservices);
    $admin_top_remove = array($adminnotifications, $registrationmoodleorg, $advancedfeatures, $grades, $analytics, $competencies, $location, $modules, $security, $appearance, $frontpage, $server, $mobileapp, $development, $assignmentupgrade, $toolmultilangupgrade, $moodleservices);
    ...
    $tooluploaduserpictures = 'tooluploaduserpictures';
    //          Courses (courses)
    // $courses = 'courses';
    ...
    $reportquestioninstances = 'reportquestioninstances';
    ...
    // $admin_remove = array('editsettings', 'turneditingonoff', 'coursereports', 'gradebooksetup', 'backup', 'restore', 'import', 'reset', 'questionbank');
	// $admin_remove = array($users, $reports);
	$admin_second_remove = array($roles, $privacy, $userdefaultpreferences, $profilefields, $tooluploaduserpictures, $addcategory, $restorecourse, $coursesettings, $courserequest, $backups, $tooluploadcourse, $comments, $reportbackups, $reportconfiglog, $reporteventlists, $reportperformance, $reportquestioninstances, $reportsecurity, $reportstats, $toolmonitorrules, $toolspamcleaner, $local_swtc_settings, $invitehistory, $servicebench, $addnewcourse);
	//****************************************************************************************
    ...
    if (isset($debug)) {
        ...
        // $messages[] = "About to print SESSION->EBGLMS->USER.";
        // $messages[] = print_r($SESSION->EBGLMS->USER, true);
        // $messages[] = "Finished printing SESSION->EBGLMS->USER.";
        // $messages[] = "SWTC ********************************************************************************.";
        //
   ...
   // SWTC ********************************************************************************
	//		Remove 'Site administration' for all users (if found; normally would not be).
    // SWTC ********************************************************************************
    // $adminroot = $settingsnav->find($siteadministration, navigation_node::TYPE_SITE_ADMIN);
    // if (!$adminroot) {
    ...
    if ($adminroot = $settingsnav->find('siteadministration', \navigation_node::TYPE_SITE_ADMIN)) {
        ...
        // if ( !empty($children)) {
            ...
            // SWTC ********************************************************************************
            // 09/05/18 - Adding check for Lenovo-administrators.
            // 11/15/18 - PremierSupport and ServiceDelivery managers and admins have special access.  Adding the "Site administration" menu.
            ...
            // 12/14/18 - Rolling back to simple Accesstype check.
            // 01/18/19 - Trying to fix this logic.
            // if (stripos($swtc_user->user_access_type, $access_ps_mgr) === false) {
            // 	print_object("in settings_navigation - compare WORKED");
            // } else {
            // 	print_object("in settings_navigation - compare did NOT work");
            // }
            ...
            // 02/13/19 - Fixed access for Lenovo-admin and Lenovo-siteadmin user types.
            // 02/15/19 - Adding moodle/site:config to Lenovo-siteadmin and Lenovo-admin so that they get access to local_course_templates
            //                      and Lenovo Services Education site menu options; remove Site administration menu options that not
            //                      all administrators need.
 ...
 * History:
 *
 * 05/21/18 - Moved most of the menu items that are checked to be enabled / disabled in the role definition; check below for more details.
 * 12/03/18 - Remember that the capabilities for Managers and Administrators are applied in the system context; the capabilities
 *                      for Students are applied in the category context.
 * 12/11/18 - If the user has not logged in yet (as in resetting password), $SESSION->EBGLMS and $SESSION->EBGLMS->USER will
 *						exist, but most values will not be set yet (because they haven't logged in yet). Therefore, we must check for this condition
 ...
 * 12/14/18 - Due to problems with contexts and user access, removing has_capability checking and rolling back to checking Accesstype for
 *						PremierSupport and ServiceDelivery user types.
 * 12/16/18 - For PremierSupport and ServiceDelivery manager / admin users, changing the default "Enrolled users" link and replacing with
 *						call to curriculums report.
 * 12/28/18 - Added 'group' parameter to URL that is built for each tab for PremierSupport and ServiceDelivery managers and administrators.
 ...
 * 02/13/19 - Fixed access for Lenovo-admin and Lenovo-siteadmin user types.
 * 02/25/19 - Course administration menu options that not all administrators need.
 * 03/03/19 - Added PS/SD site administrator user access types.
 * 03/06/19 - Added PS/SD GEO administrator user access types.
 * 05/09/19 - Added additional code for Lenovo-admins and Lenovo-siteadmins; handle case if groupid is empty.
 * 05/13/19 - Updated local_swtc_extend_navigation_course to correct the course participation hyperlink (changed "course" to "id").
 * 10/22/19 - For Lenovo administrators, adding back in several Course Administration and Site Administration menu items.
 * 11/06/19 - Lenovo EBGLMS for Moodle 3.7+; changes to set Lenovo swtc_user and debug variables.
 *
 **/
function local_swtc_extend_navigation_course(navigation_node $parentnode, stdClass $course, context_course $context) {
	global $USER, $PAGE, $CFG, $DB, $SESSION;
	...
	$courseparticipationnode = 'courseparticipation';
	$courseparticipation = 'Course participation';					// The actual "Course participation" string itself.

	$activitycompletionnode = 'activitycompletion';
	$activitycompletion = 'Activity completion';					// The actual "Activity completion" string itself.

	$admin_change = array($review, $coursecompletionnode, $courseparticipationnode, $activitycompletionnode);
	$curriculumid = null;
    ...
    if ( !empty($usersnode)) {
		...
		// SWTC ********************************************************************************
        // 11/15/18 - PremierSupport and ServiceDelivery managers and admins have special access. For now, remove everything
        //      but "Enrolled users" and "Enrollment methods".
        // 12/03/18 - Remember that the capabilities for Managers and Administrators are applied in the system context;
        //                          the capabilities for Students are applied in the category context.
		// 12/14/18 - Rolling back to simple Accesstype check.
		// 01/18/19 - Trying to fix this logic.
		// if (stripos($swtc_user->user_access_type, $access_ps_mgr) !== false) {
		// 	print_object("in navigation_course - compare WORKED");
		// } else {
		// 	print_object("in navigation_course - compare did NOT work");
		// }
        // 01/24/19 - Due to the updated PremierSupport and ServiceDelivery user access types,
        // using preg_match to search for access types.
        // 02/13/19 - Fixed access for Lenovo-admin and Lenovo-siteadmin user types (i.e. don't add types to check).
        // 03/03/19 - Added PS/SD site administrator user access types.
        // 03/06/19 - Added PS/SD GEO administrator user access types.
        // 05/09/19 - Added additional code for Lenovo-admins and Lenovo-siteadmins.
		// SWTC ********************************************************************************
        if ((!preg_match($access_ps_mgr, $user_access_type)) && (!preg_match($access_ps_admin, $user_access_type)) && (!preg_match($access_ps_geoadmin, $user_access_type)) && (!preg_match($access_ps_siteadmin, $user_access_type)) && (!preg_match($access_lenovo_sd_mgr, $user_access_type)) && (!preg_match($access_lenovo_sd_admin, $user_access_type)) && (!preg_match($access_lenovo_sd_geoadmin, $user_access_type)) && (!preg_match($access_lenovo_sd_siteadmin, $user_access_type)) && (!preg_match($access_lenovoadmin, $user_access_type)) && (!preg_match($access_lenovositeadmin, $user_access_type))) {
			if (isset($debug)) {
			...
			$user_is_enrolled_this_course = is_enrolled($context) ? true : false;

			// Next, see if this course is part of ANY curriculum. If so, which one(s).
			//		Note: curriculums will have list of course id's.
			$course_partof_curriculum = curriculum_courses_find_course($course->id);
			...
			if (isset($debug)) {
				$messages[] = "user_is_enrolled_this_course is :$user_is_enrolled_this_course.===4.2.4===.";
				$messages[] = "course_partof_curriculum follows :";
				$messages[] = print_r($course_partof_curriculum, true);
			...
			//
			// SWTC ********************************************************************************
			if (!isset($course_partof_curriculum) || !$user_is_enrolled_in_curriculum_course || !$user_is_enrolled_this_course) {
                // SWTC ********************************************************************************
